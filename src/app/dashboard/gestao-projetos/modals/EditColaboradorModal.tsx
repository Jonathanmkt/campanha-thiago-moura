...
interface UpdateColaboradorData {
  funcao: string
  especializacao?: string
  nivel_acesso_projetos: string
  pode_criar_projetos: boolean
...
interface EditColaboradorModalProps {
  ...
  onUpdateColaborador: (colaboradorId: string, colaboradorData: UpdateColaboradorData) => Promise<void>
}
...
export function EditColaboradorModal({ open, onOpenChange, colaborador, onUpdateColaborador }: EditColaboradorModalProps) {
  const [formData, setFormData] = useState<UpdateColaboradorData>({
    funcao: '',
    especializacao: '',
  ...
  useEffect(() => {
    if (colaborador) {
      setFormData({
        ...
        especializacao: colaborador.especializacao || '',
        nivel_acesso_projetos: colaborador.nivel_acesso_projetos,
        pode_criar_projetos: colaborador.pode_criar_projetos,
        pode_gerenciar_tarefas: colaborador.pode_gerenciar_tarefas,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!colaborador || !formData.funcao) {
    ...
    try {
      await onUpdateColaborador(colaborador.id, formData)
      onOpenChange(false)
    } catch (error) {
  ...
  const handleInputChange = (field: keyof UpdateColaboradorData, value: string | boolean) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
