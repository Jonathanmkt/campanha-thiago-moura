...
import { Eye, EyeOff, AlertCircle, Loader2, CheckCircle2, Check, X } from 'lucide-react'
...
function getErrorMessage(errorCode: string): string {
  ...
  return errorMessages[errorCode] || 'Ocorreu um erro ao criar sua conta. Tente novamente.';
}
...
export function SignUpForm() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!password) {
      setFormError({ field: 'password', message: 'Por favor, crie uma senha.' })
      return false
    }
    
    if (password.length < 6) {
      setFormError({ field: 'password', message: 'A senha deve ter pelo menos 6 caracteres.' })
      return false
    }
    
    if (!confirmPassword) {
      setFormError({ field: 'confirmPassword', message: 'Por favor, confirme sua senha.' })
      return false
    }
    
    if (password !== confirmPassword) {
      setFormError({ field: 'confirmPassword', message: 'As senhas não coincidem.' })
      return false
    }
        ...
        <div className="grid gap-4">
          ...
          <form onSubmit={handleEmailSignUp} className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <div className="relative">
                <Input
                  ...
                  aria-describedby={formError?.field === 'password' ? 'password-error' : 'password-requirements'}
                />
                <Button
                  type="button"
                  variant="ghost"
            ...
            <div className="grid gap-2">
              ...
              <div className="relative">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (formError?.field === 'confirmPassword') clearMessages()
                  }}
                  className={formError?.field === 'confirmPassword' ? 'border-red-500 focus-visible:ring-red-500 pr-10' : 'pr-10'}
                  disabled={isLoading}
                  aria-invalid={formError?.field === 'confirmPassword'}
                  aria-describedby={formError?.field === 'confirmPassword' ? 'confirm-password-error' : undefined}
                />
                <Button
                  type="button"
                  variant="ghost"
              ...
              {confirmPassword && !formError?.field && (
                <p className={`text-xs flex items-center gap-1 ${password === confirmPassword ? 'text-green-600' : 'text-amber-600'}`}>
